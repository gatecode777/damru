...
test("online fulfilment is blocked until server-confirmed payment while COD remains eligible", () => {
  ...
  // A COD order whose cash was recorded as not collected is not eligible.
  assert.equal(isOrderPaymentEligible({ paymentMethod: "cod", paymentStatus: "failed" }), false);
  assert.match(getOrderStatusPaymentError({ paymentMethod: "cod", paymentStatus: "failed" }, "delivered") || "", /Cash on Delivery/);
});
...
test("a delivered but unpaid online order cannot issue a base order reward", async () => {
  ...
  try {
    const unpaid = await awardOrderEarnings(order._id);
    assert.equal(unpaid.skipped, "NOT_ELIGIBLE");
    ...
    const paid = await awardOrderEarnings(order._id);
    assert.equal(paid.skipped, undefined);
    assert.ok(paid.credited > 0);
