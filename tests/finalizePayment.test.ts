...
test("computePayableAmount nets out a Damru redemption at the rate it was recorded at", async () => {
  await connectDB();
  const order = await makeOrder({ total: 950 });
  const damruAmount = 200; // 200 × 25 paise = ₹50, regardless of today's configured rate
  ...
  try {
    assert.equal(await computePayableAmount(order), 900);
  } finally {
    await Order.deleteOne({ _id: order._id });
...
test("computePayableAmount values a legacy redemption without a snapshot at the configured rate", async () => {
  await connectDB();
  const order = await makeOrder({ total: 950 });
  const { paisePerDamru } = await getDamruConfig();
  const damruAmount = 200;
  ...
  try {
    const payable = await computePayableAmount(order);
    assert.equal(payable, (95000 - damruAmount * paisePerDamru) / 100);
  } finally {
    await Order.deleteOne({ _id: order._id });
    await DamruTransaction.deleteOne({ _id: txn._id });
  }
