...
test("repeated earn, redeem, reversal, and refund activity creates a high refund alert", async () => {
  ...
  try {
    ...
    for (let index = 0; index < 3; index++) {
      ...
      await RewardReversal.create({ userId: user._id, originalTransactionId: credit._id, orderId: orders[index]._id, amount: 500, walletAmount: 500, debtAmount: 0, reason: "FULL_REFUND", triggerId: `risk-refund:${index}`, status: "APPLIED", idempotencyKey: `risk-reversal:${credit._id}:${Math.random()}` });
...
test("frequent reward reversals create one bounded review event without changing ledger state", async () => {
  ...
  try {
    ...
    for (let index = 0; index < 3; index++) {
      ...
      const reversal = await RewardReversal.create({ userId: user._id, originalTransactionId: credit._id, amount: 300, walletAmount: 300, debtAmount: 0, reason: "FULL_REFUND", triggerId: `risk-frequency:${credit._id}`, status: "APPLIED", idempotencyKey: `risk-frequency:${credit._id}` });
