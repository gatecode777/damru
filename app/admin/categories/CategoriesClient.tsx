...
export default function CategoriesClient({ categories }: Props) {
  ...
  // ── Filtered list ─────────────────────────────────────────────
  const filtered = useMemo(() => categories.filter(cat => {
    ...
    if (search.trim()) {
      ...
      if (!cat.name.toLowerCase().includes(q) &&
          ...
          !cat.description?.toLowerCase().includes(q)) return false;
    }
    return true;
    ...
    <>
      ...
      <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
        {[
          ...
          <button
            key={t.k}
            className={`cat-tab${statusFilter === t.k ? " tab-active" : ""}`}
            onClick={() => { setStatusFilter(t.k as "all" | "active" | "inactive"); setPage(1); clearSel(); }}
          >
            {t.l}
            <span className="cat-tab-count">{t.n}</span>
          </button>
        ))}
      </div>
                    ...
                    paginated.map(cat => {
                      ...
                      return (
                        <tr key={cat._id} className={isChecked ? "row-selected" : ""}>
                          ...
                          <td>
                            <span className="sort-badge">{cat.sortOrder}</span>
                          </td>
                          <td>
