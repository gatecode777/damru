...
export interface AchievementSummaryEntry {
  id: string;
  name: string;
...
export interface MissionSummaryEntry {
  id: string;
  name: string;
  progress: number;
  target: number;
...
export interface LoyaltyTierSummary { id: string; code: string; name: string; rank: number; badgeName: string | null; badgeIcon: string | null; damruMultiplier: number; }
export interface LoyaltyDashboardSummary { currentTier: LoyaltyTierSummary | null; nextTier: (LoyaltyTierSummary & { minimumValue: number }) | null; progressPercentage: number; remainingValue: number; }
export interface LoyaltyResponse { currentTier: LoyaltyTierSummary | null; qualification: { type: string; currentValue: number } | null; nextTier: (LoyaltyTierSummary & { minimumValue: number }) | null; progress: { percentage: number; remainingValue: number }; benefits: string[]; tiers: (LoyaltyTierSummary & { minimumValue: number; maximumValue: number | null; benefits: string[] })[]; }
...
export interface RewardsUpcoming {
